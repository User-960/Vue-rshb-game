<template>
  <div :class='styles.rulesBlockWrapper' v-if='GET_RULES_BLOCK_PS'>
    <div :class='styles.rulesBlockGame'>
      ...
      <div :class='styles.contentText'>
        <p>
          Датчики автоматизированной системы отследили растущее число вредителей на участках. Самое время применить дрон, чтобы эффективно распылять защитные средства от насекомых управляя им удалённо.
        </p>
        <br/>
        <p>
          Когда индикатор вредителей покраснеет, то для начала выбери соответствующее вредителям на участке средство, активируй дрон и кликни на участок. Необходимо вовремя успеть отправить дрон, иначе растения погибнут.
        </p>
      ...
      <div :class='styles.btnWrapper'>
        ...
        <button :class='styles.btnStartGame' @click='startGame' data-testid='rulesBlockBtnPS'>
          Играть
        </button>
      </div>
...
export default Vue.extend({
  ...
  computed: {
    ...mapGetters([EN_PestControlGameGetters.GET_RULES_BLOCK_PS]),
  },
  methods: {
    ...mapMutations([
